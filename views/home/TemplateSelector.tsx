...
const getTemplateAuthor = (tpl: GlobalTemplate, lang: string): string => {
    ...
    return lang === 'es' ? 'GainsLab' : 'GainsLab';
};
...
export const TemplateSelector: React.FC<Props> = ({
    ...
    const filteredTemplates = useMemo(() => {
        ...
        return source.filter(tpl => {
            ...
            return title.includes(q) || description.includes(q) || author.toLowerCase().includes(q);
        });
    }, [authorFilter, dayFilter, globalTemplates, lang, personalTemplates, search, tab]);
                ...
                <div className="mx-auto grid w-full max-w-xl grid-cols-3 gap-1 rounded-xl bg-[rgb(var(--surface-raised))] p-1">
                    {tabs.map(item => (
                        <button
                            ...
                            className={`min-h-10 rounded-lg px-2 text-xs font-bold transition-colors ${tab === item.id ? 'bg-primary-500 text-black' : 'text-[rgb(var(--text-muted))]'}`}
                        >
                        ...
                        <>
                            <div className="rounded-2xl border border-primary-500/24 bg-primary-500/[0.055] p-4">
                                ...
                                <p className="mt-3 text-sm leading-relaxed text-[rgb(var(--text-secondary))]">
                                    {lang === 'es'
                                        ? '12 semanas, 3 bloques y progresión prescrita. GainsLab mantiene la estructura oficial durante toda la ejecución.'
                                        : '12 weeks, 3 blocks and prescribed progression. GainsLab preserves the official structure throughout the run.'}
                                </p>
                                <button type="button" onClick={() => setShowKongDetail(true)} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary-500 px-4 text-sm font-black text-black active:scale-[0.99]">
                                    {lang === 'es' ? 'Ver programa' : 'View program'} <Icon name="ChevronRight" size={17} />
                                </button>
                            </div>

                            <div className="rounded-2xl border border-dashed border-[rgb(var(--border-strong))] p-5 text-center text-sm text-[rgb(var(--text-muted))]">
                                {lang === 'es'
                                    ? 'Los próximos sistemas estructurados aparecerán aquí. Las rutinas sueltas viven en la pestaña Rutinas.'
                                    : 'Future structured systems will appear here. Standalone routines live under Routines.'}
                            </div>
                        </>
                    ) : (
                        <>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder={tab === 'mine' ? (lang === 'es' ? 'Buscar mis rutinas…' : 'Search my routines…') : (lang === 'es' ? 'Buscar rutina o autor…' : 'Search routine or author…')}
                                    className="h-12 w-full rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] pl-10 pr-4 text-sm font-semibold outline-none placeholder:text-[rgb(var(--text-muted))] focus:border-primary-500/50 focus:ring-2 focus:ring-primary-500/10"
                                />
                            </div>
                            ...
                            {tab === 'routines' && authors.length > 1 && (
                                <label className="flex items-center gap-3 rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised)/0.7)] px-3 py-2.5">
                                    ...
                                    <select value={authorFilter} onChange={e => setAuthorFilter(e.target.value)} className="min-w-0 flex-1 bg-transparent text-right text-sm font-bold text-[rgb(var(--text-primary))] outline-none">
                            ...
                            {tab === 'mine' && (
                                <button type="button" onClick={onCreateCustom} className="flex w-full items-center gap-3 rounded-xl border border-dashed border-[rgb(var(--border-strong))] bg-[rgb(var(--surface-raised)/0.55)] p-4 text-left active:scale-[0.99]">
                                    ...
                                    <span className="min-w-0 flex-1">
                                        <span className="block text-sm font-black">{lang === 'es' ? 'Crear rutina' : 'Create routine'}</span>
                                        <span className="mt-0.5 block text-xs text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Días, ejercicios y series desde cero.' : 'Build days, exercises and sets from scratch.'}</span>
                                    </span>
                            ...
                            <div className="space-y-2">
                                {filteredTemplates.map(tpl => {
                                    const author = getTemplateAuthor(tpl, lang);
                                    return (
                                        <button key={tpl.id} type="button" onClick={() => onSelectTemplate(tpl)} className="w-full rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised)/0.72)] p-4 text-left transition-colors active:bg-[rgb(var(--surface-elevated))]">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0 flex-1">
                                                    <h4 className="truncate text-base font-black tracking-tight">{getTranslated(tpl.title, lang as any)}</h4>
                                                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] font-bold text-[rgb(var(--text-muted))]">
                                                        <span>{tpl.program.length} {lang === 'es' ? 'días' : 'days'}</span>
                                                        {tab !== 'mine' && <><span>·</span><span>{author}</span></>}
                                                    </div>
                                                </div>
                            ...
                            {filteredTemplates.length === 0 && (
                                <div className="rounded-2xl border border-dashed border-[rgb(var(--border-subtle))] px-6 py-10 text-center">
                                    <Icon name="Search" size={22} className="mx-auto text-[rgb(var(--text-muted))]" />
                                    <p className="mt-3 text-sm font-black">{lang === 'es' ? 'Sin resultados' : 'No matches'}</p>
                                    <p className="mt-1 text-xs text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Prueba otro nombre, autor o cantidad de días.' : 'Try another name, author or day count.'}</p>
                                </div>
                            )}
                        </>
                    )}
