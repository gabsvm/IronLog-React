...
export const HomeView: React.FC<HomeViewProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const completedDays = new Set(currentCycleLogs.filter(log => !log.skipped).map(log => log.dayIdx).filter(dayIdx => dayIdx >= 0 && dayIdx < structuredDefinition.daysPerWeek));
        if (completedDays.size >= structuredDefinition.daysPerWeek) return;
        const resolvedDays = new Set(currentCycleLogs.map(log => log.dayIdx).filter(dayIdx => dayIdx >= 0 && dayIdx < structuredDefinition.daysPerWeek));
        ...
        setActiveMeso(prev => prev && prev.id === mesoId && prev.week === completedCycle ? { ...prev, week: prev.week + 1, isDeload: false } : prev);
    ...
    useEffect(() => {
        ...
        const normalizeProductLabels = () => {
            ...
            root.querySelectorAll<HTMLElement>('span').forEach(node => {
                ...
                if (isKong && lang === 'es' && /^KONG\s*·\s*BLOCK\s+\d+$/i.test(text)) node.textContent = text.replace(/BLOCK/i, 'BLOQUE');
                if (isStructured && structuredDefinition && /^\d+%\s+(DONE|COMPLETADO)$/i.test(text)) {
                    node.textContent = lang === 'es' ? `${structuredCycleResolvedCount} / ${structuredDefinition.daysPerWeek} sesiones` : `${structuredCycleResolvedCount} / ${structuredDefinition.daysPerWeek} workouts`;
                }
                if (text === String(t.tapToStart || '')) node.textContent = lang === 'es' ? 'Empezar' : 'Start workout';
            });
            ...
            if (isStructured && structuredDefinition) {
                ...
                root.querySelectorAll<HTMLElement>('h4').forEach(node => {
                    const text = (node.textContent || '').trim();
                    if (/^(Weekly Timeline|Cronograma Semanal)$/i.test(text)) node.textContent = structuredDefinition.cadence?.unit === 'cycle' ? (lang === 'es' ? 'Este ciclo' : 'This cycle') : (lang === 'es' ? 'Esta semana' : 'This week');
                });

                const nextCard = root.querySelector<HTMLElement>('#tut-up-next');
                if (nextCard) {
                    const badgeContainer = Array.from(nextCard.querySelectorAll<HTMLElement>('div')).find(node => node.classList.contains('flex') && node.classList.contains('flex-wrap') && node.classList.contains('gap-2'));
                    if (badgeContainer) {
                        const seen = new Set<string>();
                        badgeContainer.querySelectorAll<HTMLElement>('span').forEach(node => {
                            const text = (node.textContent || '').trim();
                            if (!text || text.startsWith('+')) return;
                            if (seen.has(text)) node.style.display = 'none'; else { seen.add(text); node.style.display = ''; }
                        });
                    }
                    ...
                    const plannedSets = (selectedDay?.slots || []).reduce((sum: number, slot: any) => sum + (Array.isArray(slot?.prescription) ? slot.prescription.length : Number(slot?.setTarget) || 3), 0);
                    const plannedMinutes = plannedSets > 0 ? Math.max(15, Math.round(plannedSets * 2.5)) : 0;
                    if (plannedMinutes > 0) nextCard.querySelectorAll<HTMLElement>('span').forEach(node => {
    ...
    const editProgramWithStructuredGuard = () => {
        const mustConvert = (isPerformance || isGuts) && !!activeMeso;
        if (!mustConvert || !activeMeso) {
            props.onEditProgram();
            return;
        }
        const programName = isGuts ? 'GUTS · Black Swordsman' : 'PERFORMANCE';
        const convert = window.confirm(lang === 'es'
            ? `${programName} usa una progresión estructurada. ¿Convertir la semana actual en una rutina personalizada? La copia dejará de seguir la progresión automática del programa.`
            : `${programName} uses structured progression. Convert the current week into a personal routine? The copy will stop following the program's automatic progression.`);
        if (!convert) return;
    ...
    const pendingDayName = pendingDay?.dayName ? (typeof pendingDay.dayName === 'object' ? pendingDay.dayName[lang] : pendingDay.dayName) : undefined;

    const finishSkippedStructured = () => {
    ...
    return (
        <div ref={rootRef} className={`product-home-polish ${isKong ? 'kong-active' : ''} ${isStructured ? 'structured-program-active' : ''} contents`}>
            {isStructured && activeMeso && structuredDefinition && <ProgramProgressStrip week={activeMeso.week} totalWeeks={structuredDefinition.durationWeeks} lang={lang} name={activeMeso.name || structuredDefinition.title}/>} 
            <HomeViewImpl {...props} startSession={startSessionWithRecovery} onEditProgram={editProgramWithStructuredGuard}/>
            {pendingPerformanceDay !== null && isPerformance && <PerformanceRecoveryGate lang={lang} sessionName={pendingDayName} onCancel={() => setPendingPerformanceDay(null)} onStart={mode => { const dayIdx = pendingPerformanceDay; setPendingPerformanceRecoveryMode(mode); setPendingPerformanceDay(null); props.startSession(dayIdx); }}/>} 
            {showSkippedFinalCompletion && activeMeso && isKong && <Suspense fallback={<div className="fixed inset-0 z-modal bg-[rgb(var(--surface-app))]"/>}><ProgramCompletionView meso={activeMeso} logs={Array.isArray(logs) ? logs : []} lang={lang} onFinish={finishSkippedStructured} onKeep={finishSkippedStructured}/></Suspense>}
            {showSkippedFinalCompletion && activeMeso && (isPerformance || isGuts) && (
                <div className="fixed inset-0 z-modal flex items-end justify-center bg-black/70 p-4 pb-safe backdrop-blur-sm sm:items-center">
                    <div className="w-full max-w-md rounded-3xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-5 text-[rgb(var(--text-primary))] shadow-2xl">
                        ...
                        <p className="mt-4 text-[9px] font-black uppercase tracking-[0.16em] text-primary-500">{isGuts ? 'Natural Hypertrophy · GUTS' : 'GainsLab PERFORMANCE'}</p>
                        <h2 className="mt-1 text-xl font-black">{isGuts ? (lang === 'es' ? 'Semana 12 resuelta' : 'Week 12 resolved') : (lang === 'es' ? 'Ciclo 8 resuelto' : 'Cycle 8 resolved')}</h2>
                        <p className="mt-2 text-xs leading-5 text-[rgb(var(--text-secondary))]">{isGuts
                            ? (lang === 'es' ? 'Llegaste al final del bloque de 12 semanas. La sesión omitida queda registrada como tal para conservar la adherencia real; puedes cerrar el bloque o revisarlo antes.' : 'You reached the end of the 12-week block. The skipped session remains recorded as skipped to preserve real adherence; you can close the block or review it first.')
                            : (lang === 'es' ? 'Llegaste al final del bloque de 8 ciclos. Una de las posiciones fue omitida, por eso la adherencia real queda preservada en el historial en vez de fingir una sesión completada.' : 'You reached the end of the 8-cycle block. One scheduled position was skipped, so real adherence remains preserved in history instead of pretending it was completed.')}</p>
                        <button type="button" onClick={finishSkippedStructured} className="mt-5 min-h-12 w-full rounded-2xl bg-primary-500 px-4 text-sm font-black text-black">{isGuts ? (lang === 'es' ? 'Finalizar GUTS' : 'Finish GUTS') : (lang === 'es' ? 'Finalizar PERFORMANCE' : 'Finish PERFORMANCE')}</button>
