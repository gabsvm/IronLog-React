            ...
            >
                <div className="mb-2 flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 truncate text-[9px] font-bold uppercase tracking-[0.12em] text-[rgb(var(--text-muted))]">
                            {unitLabel} {safeWeek} / {resolvedTotal} · {phaseLabel}
                        </div>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                        <span className="text-xs font-black tabular-nums text-primary-500">{Math.round((safeWeek / resolvedTotal) * 100)}%</span>
                        <Icon name="ChevronRight" size={15} className="text-[rgb(var(--text-muted))]" />
                    </div>
                </div>

                <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${Math.max(1, resolvedTotal)}, minmax(0, 1fr))` }}>
                    {segments.map((segment) => {
                        const complete = segment < safeWeek;
                        const current = segment === safeWeek;
                        return (
                            <span
                                key={segment}
