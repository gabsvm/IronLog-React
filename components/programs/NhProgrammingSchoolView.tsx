...
export const NhProgrammingSchoolView: React.FC<Props> = ({ lang, templates, onBack, onSaveTemplate }) => {
  ...
  const isFit = (id: string) => fitByExercise[id] === 'works';
  ...
  const builderStatus = useMemo(() => {
    ...
    const experiencedSlots = chosenSlots.filter(slot => isExperienced(String(slot.exerciseId)));
    const fitSlots = chosenSlots.filter(slot => isFit(String(slot.exerciseId)));
    return {
      slots: slots.length,
  ...
  const updateLabTemplate = (updated: GlobalTemplate) => {
    setPersonalTemplates(prev => prev.map(item => item.id === updated.id ? updated : item));
  };
  ...
  const renderHome = () => (
    <div className="space-y-5">
      <section className="rounded-3xl border border-primary-500/30 bg-gradient-to-br from-primary-500/15 to-[rgb(var(--surface-raised))] p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-[rgb(var(--text-secondary))]">{lang === 'es' ? 'No es otra fábrica de rutinas. Aprendés a entender, decidir, probar y modificar tu propio sistema con el logbook como feedback.' : 'This is not another routine factory. You learn to understand, decide, test and modify your own system with the logbook as feedback.'}</p>
      ...
      <div className="space-y-2">
        <LevelCard icon="BookOpen" title={lang === 'es' ? 'Aprender los 4 niveles' : 'Learn the 4 levels'} subtitle={lang === 'es' ? '85%, evolving reps/sets, frecuencia, deloads, plateaus, sobrecarga y selección de ejercicios.' : '85%, evolving reps/sets, frequency, deloads, plateaus, overload and exercise selection.'} onClick={() => setScreen('learn')}/>
        ...
        <LevelCard icon="Layers" title="Hypertrophy Iceberg" subtitle={lang === 'es' ? 'Mapa curricular para no aprender 40 variables a la vez.' : 'Curriculum map so you do not learn 40 variables at once.'} onClick={() => setScreen('iceberg')}/>
        <LevelCard icon="BarChart2" title="MASSterplans" subtitle={lang === 'es' ? 'Espalda, hombros y antebrazos como marcos progresivos, no recetas.' : 'Back, shoulders and forearms as progressive frameworks, not recipes.'} onClick={() => setScreen('massterplans')}/>
        <LevelCard icon="Search" title={lang === 'es' ? 'Analizar / modificar una rutina' : 'Analyze / modify a routine'} subtitle={lang === 'es' ? 'Primero explicás qué problema querés resolver; después tocás una copia.' : 'First state the problem you are trying to solve; then touch a copy.'} onClick={() => setScreen('analyze')}/>
        <LevelCard icon="Plus" title={lang === 'es' ? 'Crear mi programa guiado' : 'Build my program with guidance'} subtitle={lang === 'es' ? 'GainsLab pone funciones; vos elegís ejercicios, experiencia, tolerancia, series y rangos.' : 'GainsLab lays out functions; you choose exercises, experience, tolerance, sets and brackets.'} onClick={() => setScreen('create')}/>
        <LevelCard icon="Repeat" title={lang === 'es' ? 'Draft → Alpha → Beta → Maduro' : 'Draft → Alpha → Beta → Mature'} subtitle={lang === 'es' ? `${labTemplates.length} programa(s) NH Lab · registrá cambios y evolución sin reescribir todo.` : `${labTemplates.length} NH Lab program(s) · record changes and evolution without rebuilding everything.`} onClick={() => setScreen('lifecycle')}/>
        <LevelCard icon="Brain" title={lang === 'es' ? 'Autoentrenarme con mi logbook' : 'Self-coach with my logbook'} subtitle={lang === 'es' ? 'Compara sólo exposiciones equivalentes y separa trabajo duro de un plateau real.' : 'Compare only like-for-like exposures and separate hard work from a real plateau.'} onClick={() => setScreen('selfcoach')}/>
      </div>
    </div>
  );
  ...
  const renderLearn = () => (
    <div className="space-y-4">
      ...
      {lessons.map(item => <section key={item.id} className="rounded-2xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-4"><EvidenceBadge kind={item.kind as NhEvidenceKind} lang={lang}/><h3 className="mt-3 text-base font-black">{item.title[lang]}</h3><p className="mt-2 text-xs leading-5 text-[rgb(var(--text-secondary))]">{item.summary[lang]}</p>{item.sourceScope && <p className="mt-3 text-[9px] font-bold uppercase tracking-[0.08em] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Fuente' : 'Source'} · {item.sourceScope}</p>}</section>)}
    </div>
  );

  const renderPath = () => (
    <div className="space-y-3">
      <section className="rounded-3xl border border-primary-500/25 bg-primary-500/[0.06] p-5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-primary-500">NH · SELF PROGRAMMING · PARTS 1–3</p><h2 className="mt-2 text-xl font-black">{lang === 'es' ? 'De depender de rutinas a construir un sistema' : 'From depending on routines to building a system'}</h2></section>
      ...
      <button type="button" onClick={() => setScreen('lifecycle')} className="min-h-12 w-full rounded-2xl bg-primary-500 px-4 text-sm font-black text-black">{lang === 'es' ? 'Abrir laboratorio Alpha/Beta' : 'Open Alpha/Beta Lab'}</button>
    </div>
  );

  const renderIceberg = () => (
    <div className="space-y-4">
      <section className="rounded-3xl border border-primary-500/25 bg-primary-500/[0.06] p-5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-primary-500">THE HYPERTROPHY ICEBERG</p><h2 className="mt-2 text-xl font-black">{lang === 'es' ? 'Aprendé por profundidad' : 'Learn by depth'}</h2><p className="mt-2 text-xs leading-5 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Es un mapa de cuándo un concepto empieza a ser útil, no un ranking científico.' : 'This maps when a concept starts becoming useful; it is not a scientific ranking.'}</p></section>
      {NH_ICEBERG_CURRICULUM.map((level,index) => <section key={level.id} className="rounded-3xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-4"><div className="flex items-center justify-between"><h3 className="text-base font-black">{level.title[lang]}</h3><span className="text-[9px] font-black text-primary-400">{index + 1}/5</span></div><p className="mt-2 text-xs leading-5 text-[rgb(var(--text-secondary))]">{level.purpose[lang]}</p><div className="mt-3 flex flex-wrap gap-2">{level.concepts.map((concept,i) => <span key={i} className="rounded-xl bg-[rgb(var(--surface-base))] px-3 py-2 text-[10px] font-bold text-[rgb(var(--text-secondary))]">{concept[lang]}</span>)}</div><p className="mt-3 text-[9px] leading-4 text-[rgb(var(--text-muted))]">{level.note[lang]}</p></section>)}
    </div>
  );

  const renderMassterplans = () => (
    <div className="space-y-4">
      <section className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-4"><p className="text-xs font-black text-amber-300">{lang === 'es' ? 'Marcos de NH, no límites universales' : 'NH frameworks, not universal limits'}</p></section>
  ...
  const renderAnalyze = () => (
    <div className="space-y-4">
      <section className="rounded-2xl border border-primary-500/20 bg-primary-500/[0.05] p-4"><h2 className="text-base font-black">{lang === 'es' ? 'Primero entendé; después modificá' : 'Understand first; modify second'}</h2><p className="mt-2 text-xs leading-5 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Elegí una rutina. El original nunca se toca: Programming School la desarma por funciones y sólo crea una copia cuando definís qué problema querés resolver.' : 'Choose a routine. The original is never touched: Programming School breaks it into functions and creates a copy only after you define the problem you are solving.'}</p></section>
      <div className="space-y-2">{templates.slice().sort((a,b) => a.order-b.order).map(template => <button type="button" key={template.id} onClick={() => { setSelectedTemplateId(template.id); setModifyReason(''); setScreen('audit'); }} className="w-full rounded-2xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-4 text-left"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-black">{String(getTranslated(template.title, lang))}</p><p className="mt-1 text-[10px] font-bold text-[rgb(var(--text-muted))]">{template.program.length} {lang === 'es' ? 'días' : 'days'}</p></div><Icon name="ChevronRight" size={16}/></div></button>)}</div>
    </div>
  );
  ...
  const renderAudit = () => {
    ...
    return <div className="space-y-4">
      <section className="rounded-3xl border border-primary-500/25 bg-primary-500/[0.06] p-5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-primary-500">NH PROGRAM AUDIT</p><h2 className="mt-1 text-xl font-black">{String(getTranslated(selectedTemplate.title, lang))}</h2><p className="mt-2 text-[10px] leading-4 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Sin nota 0–100: el objetivo es saber qué revisar, no perseguir un score arbitrario.' : 'No 0–100 grade: the objective is to know what to inspect, not chase an arbitrary score.'}</p></section>
      <div className="grid grid-cols-2 gap-2">{categories.map(category => <div key={category.id} className={`rounded-2xl border p-3 ${category.state === 'clear' ? 'border-emerald-500/20 bg-emerald-500/[0.05]' : category.state === 'change' ? 'border-rose-500/20 bg-rose-500/[0.05]' : 'border-amber-500/20 bg-amber-500/[0.05]'}`}><div className="flex items-center justify-between"><p className="text-[10px] font-black">{category.title[lang]}</p><span className="text-[8px] font-black uppercase">{category.state === 'clear' ? 'OK' : category.state === 'change' ? (lang === 'es' ? 'CAMBIO' : 'CHANGE') : (lang === 'es' ? 'REVISAR' : 'REVIEW')}</span></div><p className="mt-2 text-[9px] leading-4 text-[rgb(var(--text-muted))]">{category.summary[lang]}</p></div>)}</div>
      ...
      <section className="rounded-2xl border border-violet-500/20 bg-violet-500/[0.05] p-4"><h3 className="text-sm font-black">{lang === 'es' ? '¿Qué problema querés resolver?' : 'What problem are you solving?'}</h3><div className="mt-3 grid grid-cols-2 gap-2">{MODIFY_REASONS.map(reason => <button key={reason.id} type="button" onClick={() => setModifyReason(reason.id)} className={`min-h-11 rounded-xl border px-3 text-left text-[10px] font-bold ${modifyReason === reason.id ? 'border-violet-500/35 bg-violet-500/15 text-violet-300' : 'border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-base))]'}`}>{reason[lang]}</button>)}</div><p className="mt-3 text-[9px] leading-4 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'La copia conserva esta razón en su changelog. La idea es evitar cambios por aburrimiento o reflejo.' : 'The copy keeps this reason in its changelog. The point is to avoid changes driven by boredom or reflex.'}</p></section>
      <button type="button" disabled={!modifyReason} onClick={createLearningCopy} className="min-h-12 w-full rounded-2xl bg-primary-500 px-4 text-sm font-black text-black disabled:opacity-40">{lang === 'es' ? 'Crear copia didáctica y abrir editor' : 'Create teaching copy and open editor'}</button>
    </div>;
  };

  const renderCreate = () => (
    <div className="space-y-5">
      <section className="rounded-3xl border border-primary-500/25 bg-primary-500/[0.06] p-5"><EvidenceBadge kind="nh_principle" lang={lang}/><h2 className="mt-3 text-xl font-black">{lang === 'es' ? 'Vos programás. GainsLab te hace preguntas.' : 'You program. GainsLab asks questions.'}</h2><p className="mt-2 text-xs leading-5 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'El siguiente paso sólo crea un esqueleto de funciones. No elige ejercicios por vos.' : 'The next step only creates a movement-function scaffold. It does not choose exercises for you.'}</p></section>
      <section><p className="mb-2 text-[10px] font-black uppercase tracking-[0.12em] text-[rgb(var(--text-muted))]">{lang === 'es' ? '1 · Marco simple' : '1 · Simple frame'}</p><div className="space-y-2">{([{days:3,en:'Full body · 3 days',es:'Full body · 3 días',descEn:'Restrictive frame; fewer places to hide unnecessary complexity.',descEs:'Marco restrictivo; menos lugares para esconder complejidad innecesaria.'},{days:4,en:'Upper / Lower · 4 days',es:'Upper / Lower · 4 días',descEn:'GainsLab teaching frame with clear upper/lower recovery boundaries.',descEs:'Marco didáctico de GainsLab con límites claros torso/pierna.'},{days:5,en:'Upper / Lower + Arms · 5 days',es:'Upper / Lower + Brazos · 5 días',descEn:'More distribution capacity, but more decisions to justify.',descEs:'Más capacidad de distribución, pero más decisiones que justificar.'}] as const).map(option => <button key={option.days} type="button" onClick={() => setDays(option.days)} className={`w-full rounded-2xl border p-4 text-left ${days === option.days ? 'border-primary-500/35 bg-primary-500/10' : 'border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))]'}`}><p className="text-sm font-black">{option[lang]}</p><p className="mt-1 text-[10px] leading-4 text-[rgb(var(--text-muted))]">{lang === 'es' ? option.descEs : option.descEn}</p></button>)}</div></section>
      <section><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[rgb(var(--text-muted))]">{lang === 'es' ? '2 · Prioridades' : '2 · Priorities'}</p><span className="text-[9px] font-bold text-[rgb(var(--text-muted))]">{priorities.length}/2</span></div><div className="grid grid-cols-2 gap-2">{MUSCLES.map(muscle => <button key={muscle} type="button" onClick={() => togglePriority(muscle)} className={`min-h-11 rounded-xl border px-3 text-left text-xs font-bold ${priorities.includes(muscle) ? 'border-primary-500/30 bg-primary-500/10 text-primary-400' : 'border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))]'}`}>{muscleLabel(muscle, lang)}</button>)}</div><p className="mt-2 text-[10px] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Priorizar asigna recursos; no intentamos especializar todo a la vez.' : 'Prioritizing allocates resources; we do not specialize everything at once.'}</p></section>
      <section><p className="mb-2 text-[10px] font-black uppercase tracking-[0.12em] text-[rgb(var(--text-muted))]">{lang === 'es' ? '3 · Nombre del borrador' : '3 · Draft name'}</p><input value={draftName} onChange={event => setDraftName(event.target.value)} className="min-h-12 w-full rounded-2xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] px-4 text-sm font-bold outline-none focus:border-primary-500/50"/></section>
      <button type="button" onClick={beginBuilder} className="min-h-12 w-full rounded-2xl bg-primary-500 px-4 text-sm font-black text-black">{lang === 'es' ? 'Construir sólo el esqueleto de funciones' : 'Build the function scaffold only'}</button>
    </div>
  );
  ...
  const renderBuilder = () => {
    ...
    return <div className="space-y-4">
      <section className="rounded-3xl border border-primary-500/25 bg-primary-500/[0.06] p-5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-primary-500">GUIDED BUILDER</p><h2 className="mt-1 text-xl font-black">{draftName}</h2><div className="mt-3 grid grid-cols-4 gap-2 text-center"><div><p className="text-lg font-black">{builderStatus.chosen}/{builderStatus.slots}</p><p className="text-[8px] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'elegidos' : 'chosen'}</p></div><div><p className="text-lg font-black">{builderStatus.experienced}/{builderStatus.slots}</p><p className="text-[8px] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'experiencia' : 'experience'}</p></div><div><p className="text-lg font-black">{builderStatus.fits}/{builderStatus.slots}</p><p className="text-[8px] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'tolerados' : 'fit'}</p></div><div><p className={`text-lg font-black ${builderStatus.ready ? 'text-emerald-400' : 'text-amber-400'}`}>{builderStatus.ready ? 'READY' : 'DRAFT'}</p><p className="text-[8px] text-[rgb(var(--text-muted))]">Alpha</p></div></div></section>
      {draft.map((day, dayIndex) => <section key={day.id} className="rounded-3xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-4"><h3 className="text-base font-black">{String(getTranslated(day.dayName, lang))}</h3><div className="mt-4 space-y-4">{(day.slots || []).map((slot, slotIndex) => {
        ...
        return <div key={`${day.id}-${slotIndex}`} className="rounded-2xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-base))] p-3">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black">{definition?.title[lang] || slot.label || slot.muscle}</p><p className="mt-1 text-[9px] leading-4 text-[rgb(var(--text-muted))]">{definition?.purpose[lang]}</p></div><span className="rounded-lg bg-primary-500/10 px-2 py-1 text-[8px] font-black text-primary-400">{definition?.systemicCost?.toUpperCase()}</span></div>
          <label className="mt-3 block text-[9px] font-black uppercase tracking-[0.08em] text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Elegí el ejercicio' : 'Choose the exercise'}</label>
          <select value={selectedId} onChange={event => setDraft(prev => prev ? updateNhDraftSlot(prev, dayIndex, slotIndex, { exerciseId: event.target.value || null }) : prev)} className="mt-1 min-h-11 w-full rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] px-3 text-xs font-bold"><option value="">{lang === 'es' ? '— Elegir —' : '— Choose —'}</option>{candidateIds.map(id => { const h = historyById.get(String(id)); return <option key={id} value={id}>{exerciseName(id)}{h?.experiencedByGainsLabHeuristic ? ` · ${lang === 'es' ? 'historial ✓' : 'history ✓'}` : ''}</option>; })}</select>
          {selectedId && <div className="mt-3 space-y-2"><div className="rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black">{lang === 'es' ? 'Experiencia con este movimiento' : 'Experience with this movement'}</p><p className="mt-1 text-[9px] text-[rgb(var(--text-muted))]">{selectedHistory ? `${selectedHistory.exposureCount} ${lang === 'es' ? 'exposiciones' : 'exposures'} · ${selectedHistory.spanDays}d` : (lang === 'es' ? 'Sin historial suficiente en GainsLab' : 'No sufficient GainsLab history')}</p></div><span className={`text-[9px] font-black ${experienced ? 'text-emerald-400' : 'text-amber-400'}`}>{experienced ? 'OK' : '?'}</span></div>{!selectedHistory?.experiencedByGainsLabHeuristic && <button type="button" onClick={() => setManualExperience(prev => ({ ...prev, [selectedId]: !prev[selectedId] }))} className={`mt-2 min-h-9 w-full rounded-xl border px-3 text-[9px] font-bold ${manualExperience[selectedId] ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-[rgb(var(--border-subtle))]'}`}>{manualExperience[selectedId] ? (lang === 'es' ? '✓ Confirmo que ya lo practicaba antes' : '✓ I confirm prior practice') : (lang === 'es' ? 'Confirmar experiencia previa fuera del logbook' : 'Confirm prior experience outside the logbook')}</button>}</div>
            <div className="rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] p-3"><p className="text-[10px] font-black">{lang === 'es' ? '¿Cómo encaja con tu cuerpo?' : 'How does it fit your body?'}</p><div className="mt-2 grid grid-cols-3 gap-1.5">{([{id:'works',en:'Works',es:'Me funciona'},{id:'irritates',en:'Irritates',es:'Me molesta'},{id:'unsure',en:'Unsure',es:'No sé'}] as const).map(option => <button key={option.id} type="button" onClick={() => setFitByExercise(prev => ({ ...prev, [selectedId]: option.id }))} className={`min-h-9 rounded-lg border px-2 text-[9px] font-bold ${fit === option.id ? option.id === 'works' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-amber-500/30 bg-amber-500/10 text-amber-400' : 'border-[rgb(var(--border-subtle))]'}`}>{option[lang]}</button>)}</div></div></div>}
          <div className="mt-3 grid grid-cols-2 gap-2"><div><label className="text-[9px] font-black uppercase text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Series' : 'Sets'}</label><div className="mt-1 flex min-h-11 items-center justify-between rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))]"><button type="button" className="h-10 w-10 font-black" onClick={() => setDraft(prev => prev ? updateNhDraftSlot(prev, dayIndex, slotIndex, { setTarget: Math.max(1, Number(slot.setTarget || 1) - 1) }) : prev)}>−</button><span className="font-black">{slot.setTarget}</span><button type="button" className="h-10 w-10 font-black" onClick={() => setDraft(prev => prev ? updateNhDraftSlot(prev, dayIndex, slotIndex, { setTarget: Math.min(6, Number(slot.setTarget || 1) + 1) }) : prev)}>+</button></div></div><div><label className="text-[9px] font-black uppercase text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Rango' : 'Bracket'}</label><select value={slot.reps || ''} onChange={event => setDraft(prev => prev ? updateNhDraftSlot(prev, dayIndex, slotIndex, { reps: event.target.value }) : prev)} className="mt-1 min-h-11 w-full rounded-xl border border-[rgb(var(--border-subtle))] bg-[rgb(var(--surface-raised))] px-3 text-xs font-black">{role ? uniqueNhRepRanges(role).map(range => <option key={range} value={range}>{range}</option>) : <option value={slot.reps}>{slot.reps}</option>}</select></div></div>
        </div>;
      })}</div></section>)}
      <section className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-4"><p className="text-[10px] font-black text-amber-300">{lang === 'es' ? 'GAINSLAB RULE · listo para Alpha' : 'GAINSLAB RULE · Alpha readiness'}</p><p className="mt-1 text-[10px] leading-4 text-[rgb(var(--text-muted))]">{lang === 'es' ? 'Guardar un borrador siempre está permitido. Para declararlo listo para Alpha exigimos ejercicio elegido, experiencia confirmada y que vos marques que el movimiento te funciona. NH no dio este checklist de software exacto.' : 'Saving a draft is always allowed. To call it Alpha-ready we require an exercise choice, confirmed experience and your own confirmation that the movement fits you. NH did not prescribe this exact software checklist.'}</p></section>
      <button type="button" onClick={() => setScreen('preview')} className="min-h-12 w-full rounded-2xl bg-primary-500 px-4 text-sm font-black text-black">{lang === 'es' ? 'Revisar mi borrador' : 'Review my draft'}</button>
    </div>;
  };
